...
export const BindingConfigSchema = z.object({
  ...
  prefix: z
    .string()
    .min(1, "绑定前缀不能为空")
    .max(50, "绑定前缀长度不能超过 50 个字符")
    .default("/绑定 "),
  unbindFailMsg: z
    .string()
    .max(200, "解绑失败消息长度不能超过 200 个字符")
    .default("解除绑定 {user} 失败！{why}"),
  unbindPrefix: z
    .string()
    .max(50, "解绑前缀长度不能超过 50 个字符")
    .default("/解绑 "),
  unbindSuccessMsg: z
    .string()
    .max(200, "解绑成功消息长度不能超过 200 个字符")
    .default("解除绑定 {user} 成功！"),
  unbindkickMsg: z
    .string()
    .max(500, "解绑踢出消息长度不能超过 500 个字符")
    .default(
      "&l&k123456&a&l社交帐号被解绑&r&l&k123456\n您的社交账号&c {social_account} &r已在社交平台解绑。",
    ),
});
