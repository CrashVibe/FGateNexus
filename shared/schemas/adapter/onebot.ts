...
export const OneBotWSConfigSchema = z.object({
  ...
  timeout: z.number().int("超时时间必须是整数").positive("超时时间必须是正整数").default(5000),
  retryTimes: z.number().int("重试次数必须是整数").nonnegative("重试次数不能为负数").default(3),
  retryInterval: z.number().int("重试间隔必须是整数").positive("重试间隔必须是正整数").default(3000),
  retryLazy: z.number().int("重试延迟必须是整数").nonnegative("重试延迟不能为负数").default(1000)
});
