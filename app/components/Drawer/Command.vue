  ...
  <n-drawer-content closable title="远程指令配置">
    <n-form :model="selectedTarget" label-width="100">
      <n-form-item label="是否开启此目标的远程指令" required>
        <n-switch
          v-model:value="selectedTarget.config.CommandConfigSchema.enabled"
        />
      </n-form-item>
      ...
      <n-form-item v-if="target.type === 'group'" label="权限">
        <n-tooltip show-arrow trigger="focus">
          <template #trigger>
            <n-select
              v-model:value="
                selectedTarget.config.CommandConfigSchema.permissions
              "
              :options="options"
              multiple
              placeholder="请选择权限"
