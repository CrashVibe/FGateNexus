    ...
    <n-drawer-content closable title="配置修改">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <template v-if="selectedType === AdapterType.Onebot && formData.config">
                        ...
                        <n-form-item label="Token" path="token">
                            <n-input v-model:value="formData.config.token" placeholder="发送信息时用于验证的字段" />
                        </n-form-item>
                        ...
                        <template v-if="formData.config.protocol === 'ws'">
                            ...
                            <n-form-item label="重试次数" path="retryTimes">
                                <n-input-number
                                    v-model:value="wsConfig.retryTimes"
                                    :min="0"
                                    :step="1"
                                    class="w-full"
                                    placeholder="3"
        ...
        <template #footer>
            <div class="flex justify-end gap-3">
                <n-button :disabled="loading" :loading="loading" ghost type="error" @click="handleDelete">
                    删除
                </n-button>
