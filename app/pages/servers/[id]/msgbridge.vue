                ...
                <UPageCard variant="outline" title="消息过滤">
                  ...
                  <template #footer>
                    <div class="flex flex-col gap-4">
                      ...
                      <template
                        v-if="
                          formData.config.filters.filterMode === 'blacklist'
                        "
                      >
                        <UFormField
...
const createArrayTextComputed = (
  ...
  computed({
    get: () => {
      ...
      return (formData.config.filters[key] || []).join(",");
    },
    set: (v: string) => {
      if (!formData.config) {
        return;
...
const mcToPlatformPreview = computed(() => {
  ...
  return formatMCToPlatformMessage(formData.config.mcToPlatformTemplate, {
...
const platformToMcPreview = computed(() => {
  ...
  return formatPlatformToMCMessage(formData.config.platformToMcTemplate, {
