...
export default defineEventHandler(async (event) => {
    try {
        ...
        const parsed = NotifyConfigSchema.safeParse(body);
        ...
        const result = await database
            .update(servers)
            .set({
                notifyConfig: parsed.data
            })
            .where(eq(servers.id, serverId))
            .returning();

        if (!result[0]) {
            const apiError = ApiError.database("更新服务器通知配置失败: 未能找到服务器");
            return createErrorResponse(event, apiError);
        }
        return createApiResponse(event, "更新服务器通知配置成功", StatusCodes.OK);
    } catch (err) {
        console.error("更新服务器通知配置失败：", err);
        const apiError = ApiError.internal("更新服务器通知配置失败");
        return createErrorResponse(event, apiError);
    }
});
