...
const playerJoinSchema = createJsonRpcRequestSchema(
  z.object({
    playerName: z.string(),
  }),
);

export class PlayerJoinHandler implements RequestHandler {
  method = "player.join";

  async handleRequest(request: JsonRpcRequest, peer: Peer): Promise<void> {
    const result = playerJoinSchema.safeParse(request);
    if (!result.success) {
